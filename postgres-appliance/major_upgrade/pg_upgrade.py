...
class _PostgresqlUpgrade(Postgresql):
    ...
    def pg_upgrade(self, check=False):
        ...
        import glob
        diag = [os.path.join(upgrade_dir, 'pg_upgrade_server.log'),
                os.path.join(self._new_data_dir, 'pg_upgrade_server.log')]
        diag += glob.glob(os.path.join(self._new_data_dir, 'pg_upgrade_output.d', '**', '*.txt'), recursive=True)
        diag += glob.glob(os.path.join(upgrade_dir, 'pg_upgrade_output.d', '**', '*.txt'), recursive=True)
        for f in diag:
            try:
                with open(f) as fh:
                    logger.error('DIAG %s:\n%s', f, fh.read())
